...
export async function getAnimalsAndGetDetails() {
    ...
    const options = animals?.map((a) => ({ title: a.name, value: a.id,size:a.size}));
...
export async function getAnimals() {
    ...
    const options = [
        ...
        { type: 'select', name: 'size', message: 'Select Animal size', choices: [{ title: 'small', value: 'small' }, { title: 'medium', value: 'medium' },{ title: 'large', value: 'large' }] }
    ]
...
export async function selectAnimalId(options: AnimalChoice[] | undefined): Promise<number> {
